...
+ (NSArray *)tableColumnNames:(Class<AcrossDataProtocol>)cls {
    ...
    NSMutableDictionary *resultDictionary = [[AcrossDataBase sharedInstance] query:queryCreateTableSqlString].firstObject;
...
+ (BOOL)isTableExist:(Class)cls {
    NSString *tableName = [cls tableName];
    NSString *queryCreateTableSqlString = [NSString stringWithFormat:@"select sql from sqlite_master where type = 'table' and name = '%@'", tableName];
    NSMutableArray *resultArray = [[AcrossDataBase sharedInstance] query:queryCreateTableSqlString];
...
+ (BOOL)createTable:(Class<AcrossDataProtocol>)cls {
    NSString *tableName = [cls tableName];
    ...
    return [[AcrossDataBase sharedInstance] execute:sqlString];
}

+ (BOOL)dropTable:(Class<AcrossDataProtocol>)cls {
    NSString *tableName = [cls tableName];
...
+ (BOOL)alterTable:(Class<AcrossDataProtocol>)cls {
    NSString *tmpTableName = [cls minimumTmpTableName];
    ...
    return [[AcrossDataBase sharedInstance] executes:sqls];
}
...
+ (BOOL)updateTable:(AcrossDataModelBase *)model {
    ...
    NSString *execSql = @"";
    if ([[AcrossDataBase sharedInstance] query:checkPrimaryKeySql].count > 0) {
    ...
    return [[AcrossDataBase sharedInstance] execute:execSql];
}
...
+ (NSArray<AcrossDataModelBase *> *)queryTable:(AcrossDataModelBase *)model where:(NSArray<NSString *> *)names {
    Class cls = [model class];
    NSString *tableName = [cls tableName];
    if (!names || !names.count) {
        return nil;
    ...
    NSArray<NSDictionary *> *results = [[AcrossDataBase sharedInstance] query:querySql];
